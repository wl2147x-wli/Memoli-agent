...
def test_resolve_prefers_raw_provider_key_over_base_key():
    # 原始提供商 ID（“claudeAPI”）与基本提供商 ID（“claude”）不同。存储的值
    # 原始 ID 下的数据必须胜过标准化基本 ID 下存储的数据。
...
def test_resolve_falls_back_to_base_provider_key():
    # 旧版/备用 id 规范化：claudeAPI -> claude.当只有底座时
    # 密钥被存储、被使用。
...
# --- R7 不变式：标准化始终输出有效的本机枚举 ---
# 每个受支持的提供商/模型，对于每个遗留的全球价值
# 跨供应商出现，必须标准化为该模型自己的值*
# 允许设置*。这可以防止重映射表默默地发出枚举
# 上游 API 拒绝，或者碰撞到模型枚举之外的层。
# 它只测试当前的能力分支——它不会重构
# 能力数据。


def test_normalization_always_outputs_valid_native_enum():
    # get_reasoning_capability 中的每个功能分支都有一个代表性模型。
    supported_pairs = [
        ("deepseek", "deepseek-v4-flash"),
        ("zhipu", "glm-5.2"),
        ("claude", "claude-fable-5"),
        ("claude", "claude-opus-4-6"),  # 仅最大克劳德变体
    ]
    # 任何地方都可以看到的每个遗留工作值的联合（本机枚举+
    # 旧版重映射输入最少/无）。
