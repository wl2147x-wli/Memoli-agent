...
def test_multi_agent_backup_restore_preserves_all_isolated_workspaces(tmp_path):
    ...
    # 相同的布局 AgentRegistry.from_config 派生：默认 Agent 拥有
    # 实例根目录，其余部分位于agents/<id>下。
    assert destinations == {
    ...
    # 花名册将在工作区旁边恢复，而不是恢复到 config.json 中，
    # 而且它根本没有命名任何路径：这个布局是注册表派生的，
    # 因此恢复的树可以再次移动而无需编辑任何内容。
