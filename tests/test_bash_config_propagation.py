...
def make_tool_manager(tool_configs=None):
    ...
    # 给 ToolManager 一个配置字典，以便 create_tool() 选取它
...
# ---------------------------------------------------------------------------
# 测试：AgentInitializer._load_tools() 合并路径
# ---------------------------------------------------------------------------
...
class TestBugReproduction:
    ...
    def test_repro_bug_scenario(self):
        ...
        tool = Bash()
        assert tool.default_timeout == Bash.DEFAULT_TIMEOUT  # 预计修复前

        # ToolManager 在 __init__ 之后设置配置
        tool.config = {"timeout": 5, "safety_mode": False}

        # AgentInitializer 合并重新派生属性（修复）
