...
def _setup_workspace():
    ...
    # 可编辑技能 3：API 调用者的 SKILL.md 硬编码了错误的端点
    # 主机。对话在运行时发现正确的主机；正确的
    # 修复是编辑此文件的源代码，而不仅仅是记录更正的事实。
...
def scenario_skill_create_implicit():
    ...
    return {
        ...
        # 在实模式下，模型自行决定。脚本化的副作用
        # 仅电线存根模式；它将捕获过程模拟为一种技能。
        "scripted": (
            "我刚做了一次自我学习。\n"
        ...
        try:
            ...
            if sc["expect_evolved"]:
                ...
                if "expect_memory_contains" in sc:
                    # Evolution 现在写入带日期的每日文件，而不是 MEMORY.md。
                    from datetime import datetime
...
def run_real():
    ...
    # 加载 config.json，以便机器人可以使用真正的 API 密钥。
    load_config()

    # 默认测试为 deepseek-v4-flash（快速、低成本），除非被覆盖。
    ...
    # 捕获面向用户的回复，而不是将其推送到频道。
    captured = {"reply": None}
    orig_notify = ex._notify_user
    ex._notify_user = lambda ct, rcv, summary: captured.__setitem__("reply", summary)

    results = []  # （名称、目标、演变、更改、reply_ok）

    only = os.environ.get("EVO_TEST_ONLY")  # 目标/名称的子字符串过滤器
    try:
        for make in SCENARIOS:
            sc = make()
            ...
            try:
                ...
                # 完全隔离的代理：工具cwd + memory_manager -> temp ws。
