...
const FileTree: React.FC = () => {
  ...
  // 初始加载，加上其他地方请求的跳转（例如单击目录
  // 消息中的引用）。计数器还覆盖了面板所在的情况
  // 已关闭，因此该组件会在请求已挂起的情况下安装。
  ...
  useEffect(() => {
    ...
    // `dir`/`searching` 为只读，用于在查询时恢复列表
    // 清除；重新运行他们的更改会影响用户的打字。
      ...
      {!searching && (
        <div className="flex items-center flex-wrap gap-0.5 px-3 pb-2 text-[11px] text-content-tertiary">
          ...
          >
            ...
            {/* 在根目录下，显示绝对根路径，以便用户知道哪个
                面板锚定到的目录（project 或 ~/cow）。 */}
