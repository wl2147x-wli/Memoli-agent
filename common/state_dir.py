...
# ``base`` 让已经带有解析根的值对象（MemoryConfig，
# KnowledgeService）重用布局，无需重新解析他们所做的身份
# 没有。它的存在是为了保证布局只定义一次。它命名为
# 代理自己的根，而不是一个独立的根：共享资产仍然回落到
# 当该根没有共享副本时，这是调用者传递它的内容
# 其实想要。


# --- 共享：每个特工都可以使用一份副本，除非它选择退出 ---------------
