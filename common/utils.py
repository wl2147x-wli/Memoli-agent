...
def compress_imgfile(file, max_size):
    ...
    while True:
        ...
        if fsize(out_buf) <= max_size or quality <= min_quality:
            # 在 min_quality 处停止：进一步递减将传递无效值
            # 质量 (<1) 到 PIL，否则循环将永远不会终止
            # 对于无法压缩到低于 max_size 的图像。
            return out_buf
        quality -= 5
...
def expand_path(path: str) -> str:
    ...
    # 如果扩展不起作用（路径仍然以 ~ 开头），请使用 HOME 或 USERPROFILE
    if expanded.startswith('~'):
        import platform
        if platform.system() == 'Windows':
            # 在 Windows 上，首先尝试 USERPROFILE，然后尝试 HOME
...
# 高于此值时，报告的内存限制意味着“无限制”，而不是实际的内存限制
# cap（当未设置限制时，内核会公开近 64 位哨兵）。
