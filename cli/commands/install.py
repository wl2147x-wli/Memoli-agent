...
# 流(msg, fg=None) — fg 是“黄色”| “绿色”| “红色”|无
StreamFn = Callable[[str, Optional[str]], None]
# on_phase(msg) — 聊天频道的粗粒度进度（通过 i18n 本地化）
...
def _playwright_cli(args: list, env: Optional[dict] = None) -> int:
    ...
    # Frozen：在进程中运行捆绑的 Playwright 驱动程序。计算驱动程序可执行文件
    # 返回捆绑包内附带的节点驱动程序；我们直接生成它。
    prev_env = {}
    if env:
    ...
    try:
        ...
        # 在较新的版本上，compute_driver_executable 可能会返回一个元组（node、cli.js）
        # 剧作家，或者是老一代的单一道路。
        if isinstance(driver, (list, tuple)):
            cmd = list(driver) + args
        else:
            cmd = [str(driver)] + args
        # get_driver_env() 快照 os.environ，我们已经对其进行了修补
        # 上面调用者的覆盖（PLAYWRIGHT_BROWSERS_PATH / DOWNLOAD_HOST），
        # 所以镜像+固定浏览器目录在这里也受到尊重。
        return subprocess.call(cmd, env=get_driver_env())
    except Exception as e:
        # 最后的手段：通过 runpy 尝试模块 main （如果冻结的构建有效
        # 保持 playwright.__main__ 可导入）。
        try:
            import runpy
...
def run_install_browser(
    ...
    # 仅在 get_cli_language() 运行 Ensure_sys_path() 后导入 `common`，
    # 因此当从项目目录外部调用 `cow` 时它会起作用。
    get_cli_language()  # 解析cow_lang，以便 i18n.t 反映配置
    ...
    # 仅限 Windows：greenlet 3.2.x 不附带 Windows 轮子，因此 pip 会构建它
    # 从源（需要 MSVC）并失败。预安装 3.1.x（具有用于
    # py3.7-3.13) 仍然满足剧作家的 greenlet>=3.1.1,<4。
    ...
    if frozen:
        # 桌面捆绑包：剧作家已内置在应用程序中；有
        # 没有 pip，也没有什么可安装的。直接跳到下载 Chromium。
    ...
    # 在 Playwright 可用的情况下，更喜欢用户的系统 Chrome/Edge：浏览器
    # 工具直接驱动它（channel =“chrome”/“msedge”），所以我们可以跳过繁重的
    # 整个 Chromium 下载大约 150MB。适用于每个运行时（桌面、Web、
    # 源）——只有无头 Linux 服务器，通常缺少系统浏览器，
    # 进入下面的下载。通过荣誉prefer_system_browser
    # solve_engine，因此强制下载 Chromium 的用户仍然可以获得它。
    try:
    ...
    if sys.platform == "linux":
        ...
        # 仅首选 fonts-wqy-zenhei（~几 MB）。 fonts-noto-cjk 要大得多（~150MB+）。
    ...
    env = os.environ.copy()
    # 固定下载位置，以便它在桌面应用程序更新和匹配中继续存在
    # 运行时查找的内容（请参阅 browser_env.browsers_download_dir()）。
    try:
