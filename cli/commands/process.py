...
def start(foreground, no_logs):
    ...
    # 仅终端设置需要交互式 tty；强制前景所以
    # 终端通道可以读取 stdin，而不是通过 tty 与 shell 进行斗争。
...
# 在调用者的进程树中幸存的分离中继。通过 `python -c` 运行
# start_new_session=True 所以它会继续追随代理的 bash 子进程
# （以及它杀死的主应用程序）都死了。流程：先自检新代码
# （导入应用程序）；如果失败，则中止而不触及旧进程。仅当
# 新代码是可加载的吗 SIGTERM 旧的 PID，等待退出（SIGKILL
# Fallback），然后启动一个新的 app.py 并写入 pid。
_RELAY_SCRIPT = r"""
...
def update(ctx):
    ...
    # 1.先停止服务，这样git pull就不会与正在运行的代码冲突
    ctx.invoke(stop)
    ...
    if _IS_WIN:
        # 在 Windows 上，`cow.exe`（此进程）锁定 exe 文件，因此
        # `pip install -e .` 失败并出现 WinError 5。编写一个小 .bat
        # 等待cow.exe退出然后安装并启动的帮助程序。
...
def status():
    ...
    # get_cli_language() 调用 Ensure_sys_path()，这会添加项目根目录
    # 到系统路径。仅在此之后导入 `common`，否则会失败
    # 当 `cow` 从项目目录外部运行时出现 ModuleNotFoundError。
