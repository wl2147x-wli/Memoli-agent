...
class MemoryFlushManager:
    ...
    def __init__(
        ...
        self._trim_flushed_hashes: set = set()  # 已刷新消息的内容哈希
        self._last_flushed_content_hash: str = ""  # 上次刷新时的内容哈希，用于每日总结去重
        self._last_dream_input_hash: str = ""  # 上次深梦使用的“{date}:{daily_hash}”，用于去重
    ...
    def flush_from_messages(
        ...
        """
        try:
            # 在进行任何后续处理前，先剔除调度程序注入的对话对。
            # 这些消息早已作为接收方会话的内部短期背景展示过，
            # 若再提升为长期每日记忆，只会产出低价值的流水账
            #（例如“11:28 价格=1013，正常/11:58 价格=1013，正常/...”），
            # 还会浪费总结所用的 token。
    ...
    def _flush_worker(
        ...
        try:
            ...
            # 如果模型仍然输出旧的 [DAILY]/[MEMORY] 标记，则将其删除
    ...
    def deep_dream(self, user_id: Optional[str] = None, lookback_days: int = 1, force: bool = False) -> bool:
        ...
        # 去重：如果今天已对相同的日常内容做过深梦，则跳过。
        # 注意：只对 daily_content 做哈希（不含 memory_content），因为
        # deep_dream 本身会重写 MEMORY.md；若把 memory_content 也计入哈希，
        # 同一时间窗口内后续每次调用都会因改写而失效。
        import hashlib
        ...
        # 调用 LLM 进行蒸馏；遇到瞬时错误（超时/连接失败/限流/5xx）
        # 或空响应时会自动重试，这样一次偶发故障不会让当晚的
        # 深梦白白丢失。
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                # 不设输出上限：提示词已让 MEMORY.md 保持精炼（约 50 条），
                # 若再加硬性 max_tokens 上限，反而可能截断较大的改写结果，
                # 因此让模型使用它默认的输出预算。
                request = LLMRequest(
    ...
    def _extract_first_meaningful_line(text: str, max_len: int = 120) -> str:
        ...
        for line in text.split("\n"):
            ...
            if not line:
                continue
            # 跳过 Markdown 标题、水平分割线、代码围栏、纯表情/符号行
            if re.match(r'^(#{1,4}\s|```|---|\*\*\*|[-*]\s*$|[^\w\u4e00-\u9fff]{1,5}$)', line):
                continue
            # 去掉行首的 Markdown 加粗/表情符号等装饰
...
def create_memory_files_if_needed(workspace_dir: Path, user_id: Optional[str] = None):
    ...
    # 在工作区根目录中创建主 MEMORY.md（初始化引导时始终需要）
    if user_id:
