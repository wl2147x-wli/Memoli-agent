...
class SkillService:
    ...
    def read_content(self, name: str) -> dict:
        ...
        result["filename"] = rel
        # 与 `source` 分开单独报告：内置技能可能有一份放在工作区里的
        # `custom` 副本——控制台需要这个字段来解释*为什么*会拒绝编辑，
        # 而仅凭 `source` 本身看不出这一点。
    ...
    def write_content(self, name: str, content: str,
        ...
        # frontmatter 里含有技能列表展示所用的名称与描述，
        # 因此保存编辑可以改变该技能在列表中的展示内容。
        self.manager.refresh_skills()
    ...
    def _add_package(self, name: str, payload: dict) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # 确定真正的内容根目录。
            # 若 zip 内只有一个顶层目录，就直接取其里面的内容，
            # 这样技能文件夹保持干净（不会多出不必要的嵌套）。
            top_items = [
