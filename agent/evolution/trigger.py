...
# 上下文压力触发阈值：实时上下文一旦超过代理 token 预算的这个比例，
# 即使还没攒够 min_turns 轮次，也会触发演化。目前保持为模块常量
# （不开放为用户配置）。兜底预算需与
# agent_initializer / config.py 保持一致（agent_max_context_tokens 默认 = 50000）。
_CONTEXT_RATIO = 0.8
