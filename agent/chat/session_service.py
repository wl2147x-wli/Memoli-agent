...
def generate_session_title(user_message: str, assistant_reply: str = "",
    ...
    try:
        ...
        # 机器人在失败时（网络错误、鉴权错误、触发限流等）通常会返回
        # completion_tokens=0，并带上 “请再问我一次吧”/“我现在有点累了”
        # 之类的哨兵文案，看到这些就应视为本次调用失败。
...
# 内置兜底配置，当 prompts.json 缺失或损坏时使用。
...
def _assemble_optimize_prompt(config: dict) -> str:
    ...
    # 列表键取 “principles”（现行写法），旧版则用 “rules”，两者皆可。
    ...
    if isinstance(rules, list) and rules:
        ...
        for rule in rules:
            if not isinstance(rule, dict):
                continue
            # 每条规则取 “guideline”（现行）或旧版 “instruction” 字段。
...
def optimize_prompt(user_input: str, context_messages: list = None) -> str:
    ...
    try:
        ...
        # 去掉 <think>...</think> 这类思考标签（思维模型输出的内容）
...
class SessionService:
    ...
    def _cancel_running(self, session_id: str, agent_id: str = None) -> None:
        ...
        try:
            ...
            # 进行中的运行是以带 agent 作用域的键注册的，因此这里
            # 不带作用域的查找不会悄悄取消任何进行中的请求。
