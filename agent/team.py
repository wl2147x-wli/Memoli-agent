...
# 组成“名册”的一组键，它们作为一个整体来读写：除了 agent 列表和
# 默认 agent 的选择之外，config 中其余内容与此无关。``channel_instances``
# 保存各渠道的身份、凭据及其绑定的 agent，是入站路由的唯一事实来源。
# 该键是可选的：缺省时，渠道启动会回退到 config.json 中的 ``channel_type``
# 加扁平凭据，因此旧版单 agent 安装可以保持原样。
...
def read(settings: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 宁可回退，也不要从“没有任何 agent”的状态启动：
        # 那样注册表只会退化成孤零零的默认 agent，控制台
        # 看起来就像所有 agent 都被删除了一样。
...
def stamp(settings: Mapping[str, Any]) -> tuple:
    ...
    try:
        ...
        # 尚无 team 文件：名册仍以 config.json 为准，
        # 因此签名也要跟着它走。
...
def migrate(settings: Mapping[str, Any], config_path: Optional[Any] = None) -> Optional[Path]:
    ...
    try:
        ...
        # 继续以 config.json 运行：它仍然可读，read() 也会
        # 回退到它，因此这里的失败损失的只是整洁，而非 agent。
