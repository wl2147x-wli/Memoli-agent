...
def build_agent_system_prompt(
    ...
    **kwargs
    ...
    # 2. 技能（紧跟工具之后，因为技能需要通过 read 工具读取）
    if skill_manager:
    ...
    # 4. Workspace（工作环境描述）。仅保留上下文文件确实加载时才生效的
    # 两个区块，子代理会跳过这一部分。
    sections.extend(
    ...
    # 4.5 权限。紧跟工作区之后，因为模型只有弄清自己工作在
    # 哪里，权限调整才有意义。完全访问模式下不输出任何内容，
    # 提示词与往常一致。
    if permission_mode:
        try:
    ...
    # 6. 项目上下文文件（AGENT.md、USER.md、RULE.md - 定义角色）
    if context_files:
...
def _build_tooling_section(tools: List[Any], language: str) -> List[str]:
    ...
    # 委派规则只有在该工具确实启用时，才值得在提示词中占一席之地。
    # 若仅凭工具描述，它会与约 30 个其他工具争夺注意力而落选：
    # 模型会直接亲自搜索，白白塞满自己的上下文。
    ...
    if is_en:
        ...
        if has_subagent:
            # 单靠工具说明无法达到这一效果：需要提示模型考虑委派。
            # 何时委派、如何委派，属于 subagent 描述里的内容；
            # 模型看到这行提示后就会去阅读该描述。
            lines.insert(
                -1,
...
def _build_knowledge_section(
    ...
    """
    # 通过 state_dir 解析路径，这样基于共享工作区的代理
    # 依然能拿到注入的共享索引，而不是空的本地索引。
    ...
    # 项目模式（以项目目录作为工作目录）下，把知识路径锚定到 ~/cow。
...
def _build_team_section(runtime_info: Dict[str, Any], language: str) -> List[str]:
    ...
    # 每行一位同事，并附一句用途说明：仅列出名字就足以
    # 向某人点名，但这并不决定某项工作是否属于他。
    roster = []
