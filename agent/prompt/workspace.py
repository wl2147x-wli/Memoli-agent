...
def ensure_workspace(workspace_dir: str, create_templates: bool = True) -> WorkspaceFiles:
    ...
    """
    # 检查这是否是一个全新的工作区（AGENT.md 尚未创建）。
    # 不能依赖“目录是否存在”来判断，因为其他模块（例如
    # ConversationStore）可能在调用本函数之前就已创建好工作空间目录。
    ...
    memory_path = os.path.join(workspace_dir, DEFAULT_MEMORY_FILENAME)  # MEMORY.md 位于根目录
    memory_dir = os.path.join(workspace_dir, "memory")  # 日常记忆子目录
    
    # 创建记忆子目录
    os.makedirs(memory_dir, exist_ok=True)

    # 技能、网站和知识在所有代理之间共享，因此它们经由 state_dir
    # 搭建，而不是建在这个工作空间下：代理靠“拥有自己的目录”来
    # 表示退出共享副本，而在这里建目录就会让每个新代理在首次
    # 启动时统统退出共享。
...
def load_context_files(workspace_dir: str, files_to_load: Optional[List[str]] = None) -> List[ContextFile]:
    ...
    if files_to_load is None:
        ...
        files_to_load = [
            ...
            DEFAULT_MEMORY_FILENAME,     # 长期记忆（冻结快照）
            DEFAULT_BOOTSTRAP_FILENAME,  # 仅在初始化尚未完成时存在
        ]
    ...
    for filename in files_to_load:
        ...
        # 自动清理：若 BOOTSTRAP.md 仍在而 AGENT.md 已填写完成，
        # 说明代理忘了删除它——此处代为清理并跳过加载
