...
class KnowledgeService:
    ...
    def __init__(self, workspace_root: str, memory_manager=None):
        ...
        # 这里用 resolve（解析）而非 join（拼接），使代理读取到的是
        # 共享的知识树，而不是自己名下那棵空的知识树。
    ...
    def _manager(self):
        if self._memory_manager is None:
            # 复用共享的嵌入提供者选择逻辑，使知识索引在同步时
            # 也能得到向量，而不是退化为仅关键字的索引。
    ...
    def rebuild_index_md(self) -> bool:
        ...
        def collect(dir_path: Path) -> list:
            # 返回 dir_path 下 *.md 排序后的 (rel_path, title) 元组，
            # 排除知识根目录里的受保护文件和以点开头的文件。
            entries = []
        ...
        def link(rel: str) -> str:
            # 对每一路径段编码，使含空格/特殊字符的路径在
            # markdown 链接中依然有效，同时保留各段之间的斜杠。
        ...
        for name in sorted(os.listdir(dir_path)):
            ...
            if os.path.isdir(full):
                ...
                if not is_root:
                    stats["pages"] += 1
                    stats["size"] += size
                # 普通文档优先采用 H1 标题作为可读标题。
                # 系统文件（index.md / log.md）仍保留文件名，
                # 使目录树不会隐藏它们的真实身份。
                title = name[:-3]
