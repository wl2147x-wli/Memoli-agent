...
class SchedulerTool(BaseTool):
    ...
    def _create_task(self, **kwargs) -> str:
        ...
        # 在创建任务时记下真实的聊天 session_id，这样调度器以后
        # 才能把已送达的输出注入到用户真正的会话中
        # （群聊里 session_id 并不等于 receiver，例如飞书上的“user_id:group_id”）。
        ...
        if message:
            ...
            # silent 仅对 ai_task 生效；固定消息始终照常发送
        ...
        # 在创建时即提示用户静默标志，便于尽早发现误设的 silent
        ...
        try:
            if schedule_type == "cron":
                ...
                if schedule_value.startswith("+"):
                    ...
                    # 绝对 ISO 时间，需转成不带时区信息的本地时间，
                    # 以便与调度器使用的 datetime.now() 保持可比。
