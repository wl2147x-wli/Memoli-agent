...
# 拼进检索查询的尾部消息条数。工具使用往往横跨多轮工具调用循环，
# 仅凭一条（最初的）用户消息并不够；取最近的一小段窗口既能捕捉
# 意图漂移，又不会用早已过时的上下文把查询撑得臃肿。
...
def _rank_by_similarity(
    ...
    if _HAS_NUMPY:
        ...
        np.maximum(denominators, 1e-10, out=denominators)                   # 避免被零除
