...
DEFAULT_TIMEOUT = 180
MAX_TOKENS = 4000
COMPRESS_THRESHOLD = 1_048_576  # 1MB
...
# 元组字段依次为：API密钥的配置键、bot_type、默认视觉模型、提供程序显示名。
# 只要配置了对应 API 密钥，这些提供者就会被自动发现，用作后备视觉提供者。
# OpenAI 和 LinkAI 走单独的原始 HTTP 通道，因此这里没有列出。
...
# 模型名称前缀 → 对应提供程序的显示名称。
# 用于把 tools.vision.model 自动路由到其原生提供程序。
# 匹配不区分大小写；取最长匹配前缀。
_MODEL_PREFIX_TO_PROVIDER = [
    ("doubao-", "Doubao"),
    ("kimi-", "Moonshot"),
    ("moonshot-", "Moonshot"),
    ("qwen", "DashScope"),       # qwen-*、qwen3-*、qwen3.6-* 等
    ("claude-", "Claude"),
    ("ernie-", "Qianfan"),
    ("gemini-", "Gemini"),
    ("glm-", "ZhipuAI"),
...
# 原生属于 OpenAI / LinkAI（原始 HTTP 提供程序）的模型前缀。
_OPENAI_MODEL_PREFIXES = ("gpt-", "o1-", "o3-", "o4-", "chatgpt-")

# 把 UI 中的提供程序 id（存于 tools.vision.provider）映射为
# VisionProvider.name 用到的显示名称。应与 _DISCOVERABLE_MODELS
# 以及 _route_by_model_name 里的 openai/linkai 分支保持一致。
...
class VisionProvider:
    ...
    use_bot: bool = False  # 为 True 时通过 bot.call_vision 调用，而不是走原始 HTTP
    fallback_bot: Any = None  # 非主模型提供商的 Bot 实例
...
class Vision(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # 默认模型只充当兜底占位符，用于那些未设置
        # VisionProvider.model_override 的提供商（例如用户未配置
        # tools.vision.model 时的原始 OpenAI 提供程序）。
    ...
    def _resolve_providers(self) -> List[VisionProvider]:
        ...
        # 第 1 步：首选提供商 — 显式的 `tools.vision.provider` 优先，
        # 这样自定义的模型名称也能被正确路由；当未设置 provider 时，
        # 则回退为按模型名称前缀推断。
        preferred = None
    ...
    def _infer_provider_from_model(model_name: str) -> Optional[str]:
        ...
        # 按前缀长度降序排序，例如“moonshot-”会胜过假设的“moo-”
    ...
    def _route_by_provider_id(self, provider_id: str, user_model: str) -> Optional[List[VisionProvider]]:
        ...
        """
        # 自定义 OpenAI 兼容提供商 — 从 custom_providers 列表读取凭证，
        # 与 embedding 使用的模式相同。
        ...
        # OpenAI / LinkAI 走原始 HTTP 提供程序，而非可自动发现的 Bot 路径。
    ...
    def _append_other_model_providers(self, providers: List[VisionProvider],
        ...
        for config_key, bot_type, default_model, display_name in _DISCOVERABLE_MODELS:
            if display_name in existing_names:
                continue
            # 与主模型同 bot_type 的提供商，通常已由
            # MainModel 提供商覆盖；若主模型确实支持视觉，
            # 就无需在此重复加入。否则，把该提供商的
            # 专用视觉模型作为后备补充进来。
    ...
    def _build_main_model_provider(self) -> Optional[VisionProvider]:
        ...
        # 采用 *当前 Agent 的* 主模型名称（支持会话级覆盖）；
        # 不要在这里注入 tools.vision.model——走到这个分支时，
        # tools.vision.model 的路由已经尝试过
        # （要么命中了主机器人，要么未能匹配到提供商）。
    ...
    def _build_openai_provider(self, preferred_model: Optional[str] = None) -> Optional[VisionProvider]:
        ...
        # 仅当 preferred_model 看起来属于 OpenAI 系列名称时才采用它；
        # 否则把供应商专有的模型名称发给 OpenAI 端点会得到 400。
    ...
    def _call_via_bot(self, model: str, question: str, image_content: dict,
        ...
        # 从 OpenAI 格式的 image_content 块中提取原始图像 URL
