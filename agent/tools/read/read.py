...
class Read(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        """
        # 支持把“location”当作“path”的别名（LLM 可能会从技能列表中使用它）
        ...
        # 安全检查：阻止读取凭证文件及其别名。
        # 参见 issue #2913（/proc/self/environ 绕过）和 #2863（读取范围）。
        if self._is_credential_path(absolute_path):
            return ToolResult.fail(DENIED_MESSAGE)
        
        # 检查文件是否存在。下面的两种报错都会直接给出明确答复，
        # 而不是含糊其辞——否则模型会倾向于猜测相近的路径，
        # 白白增加更多失败的读取。
    ...
    def _read_image(self, absolute_path: str, file_ext: str) -> ToolResult:
        ...
        try:
            ...
            # 返回图片的元数据（这里不直接放 file_to_send，真正发送请用 send 工具）
            result = {
                "type": "image_metadata",
                "file_type": "image",
                "path": absolute_path,
    ...
    def _read_text(self, absolute_path: str, display_path: str, offset: int = None, limit: int = None) -> ToolResult:
        ...
        try:
            ...
            # 读取文件（Windows 上 utf-8-sig 会自动去除 BOM）
            # 注：截断统一交由 truncate_head 处理（DEFAULT_MAX_LINES / DEFAULT_MAX_BYTES），
            # 这样 offset/limit 才能正确地对整个文件进行分页。
            ...
            # 记录本次读取，这样当文件稍后被改动时，编辑/写入工具能据此发出警告。
