...
def _import_optional_tools():
    ...
    # WebSearch 工具（初始化时按 API 密钥是否可用决定是否加载）
    try:
    ...
    # Vision Tool（按 API 密钥是否可用决定是否加载）
    try:
...
# BrowserTool：playwright 是在 browser_service 内部软导入的，因此即使
# 没有安装 playwright，这里的导入也总能成功。就绪性检查（playwright 包 /
# 系统 Chrome / 已下载的 Chromium）会在调用时于 BrowserTool 内部进行。
