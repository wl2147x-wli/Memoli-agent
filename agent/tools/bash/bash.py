...
class Bash(BaseTool):
    ...
    # cmd.exe 的命令行长度限制约为 8191 个字符；超过该长度就重写 python -c。
    _WIN_CMD_SAFE_LEN = 7000
    # 提前结束的命令会提前返回，因此把默认值设大并不会
    # 产生任何代价，模型在每次安装或构建时还可以放心重试。
    # 最大值是等待结果的时间上限；常驻型进程是另一码事，
    # 应该交给 run_in_background 处理。
    DEFAULT_TIMEOUT = 120
    MAX_TIMEOUT = 600
...
- For destructive commands out of workspace ({{cwd}}), explain and confirm first"""
    ...
    def __init__(self, config: dict = None):
        ...
        # 保留带 {cwd} 占位符的原始模板，这样当工作目录变化时
        # （例如打开了某个项目），描述文本可以随之重新渲染。
        self._description_template = self.description
        # 桌面运行在用户自己的机器上（通常是非技术用户），
        # 因此需要对工作区之外的破坏性操作进行明确确认。
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # 把产物输出锚定到工作区/项目目录，不受命令内任何 `cd` 的影响，
            # 这样工具（例如图像生成）能解析到稳定的输出目录，
            # 而不必依赖运行时不断变化的 cwd。
            if self.cwd:
            ...
            # getuid() 仅存在于类 Unix 系统上
            if hasattr(os, 'getuid'):
                logger.debug(f"[Bash] Process UID: {os.getuid()}")
            else:
                logger.debug(f"[Bash] Process User: {os.environ.get('USERNAME', os.environ.get('USER', 'unknown'))}")
            
            # 为超长的 `python -c` 命令而写的临时脚本（仅限 Windows），
            # 执行完毕后再清理。
            temp_script_path = None

            # 在 Windows 上，将 cmd.exe 的 $VAR 引用转换为 %VAR%
            if self._IS_WIN:
                env["PYTHONIOENCODING"] = "utf-8"
                command = self._convert_env_vars_for_windows(command, dotenv_vars)
                # cmd.exe 的命令行长度限制约为 8191 个字符。超长的
                # `python -c "..."` 命令会悄悄失败，因此把放不下的
                # 内联代码写入临时 .py 文件再运行它。
            ...
            if run_in_background:
                # temp_script_path 的所有权移交给后台作业注册表——
                # 后台进程可能仍在读取该文件，因此只能等作业结束后再清理。
                ...
                try:
                    ...
                    if len(parts) > 0:
                        ...
                        if retry_result.returncode == 0 or retry_result.stdout or retry_result.stderr:
                            ...
                            # 两次尝试均失败 - 检查这是否是 openai-image-vision 技能
            ...
            # 当命令通过 stdout 成功时，保持输出干净（stderr 仅进入服务器日志）。
            # 当命令失败或标准输出为空时，请包含标准错误，以便代理可以进行诊断。
            ...
            # 检查我们是否需要将完整输出保存到临时文件
            temp_file_path = None
            total_bytes = len(output.encode('utf-8'))

            if total_bytes > DEFAULT_MAX_BYTES:
                # 把完整输出保存到临时文件。这里必须用 encoding='utf-8'：
                # 文本模式的默认编码是平台区域设置（例如中文 Windows 上的
                # cp936/GBK），一旦输出包含表情符号等非本地区域字符，
                # 就会抛出 UnicodeEncodeError，从而丢掉原本成功的命令结果。
    ...
    def _get_safety_warning(self, command: str) -> str:
        ...
        """
        # 进行标记以避免子字符串误报（例如 `rm -rf /tmp/x`
        # 不得匹配 `rm -rf /`）。
        tokens = command.lower().split()

        # `rm -rf /` 或 `rm -rf /*` 指向的是真正的根目录。
    ...
    def _rewrite_long_python_c(command: str):
        ...
        """
        # 匹配：<python|python3|py> [标志] -c "<code>" （单引号或双引号）
        m = re.search(
