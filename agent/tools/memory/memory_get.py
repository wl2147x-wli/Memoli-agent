...
class MemoryGetTool(BaseTool):
    ...
    def execute(self, args: dict):
        ...
        try:
            ...
            # 若路径不以 memory/、knowledge/ 或绝对路径 / 开头，且不是根目录的 MEMORY.md，
            # 则自动补上 memory/ 前缀。
            ...
            # 使用 os.path.realpath + os.sep 做跨平台路径校验。
            # Path 在 Windows 上用的是反斜杠，str(Path).startswith(str + '/') 会失效，
            # MemoryService._resolve_path 采用了同样的做法，可作参考。
