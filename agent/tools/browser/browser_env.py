...
# “system-chrome”模式下我们接受的 Playwright 浏览器通道，
# 按优先级排列。“chrome”涵盖稳定的 Google Chrome；“msedge”
# 则是每台 Windows 10/11 都自带的基于 Chromium 的 Edge。
...
def has_downloaded_chromium() -> bool:
    ...
    try:
        for name in os.listdir(download_dir):
            # Playwright 将其浏览器目录命名为“chromium-1140”，
...
def resolve_engine(config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # auto：优先使用系统 Chrome，其次再用已下载的 Chromium。
    if system:
        return {
            "mode": "system-chrome",
            "channel": system["channel"],
