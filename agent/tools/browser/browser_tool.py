...
# 云元数据端点即使不属于链路本地地址，也应被阻止。
# （169.254.169.254 —— AWS/GCP/Azure IMDS —— 已被 is_link_local 覆盖；
# fd00:ec2::254 是 AWS 的 IPv6 IMDS 地址。）
_CLOUD_METADATA_IPS = frozenset({ipaddress.ip_address("fd00:ec2::254")})

# 启动浏览器引擎并渲染真实页面所需的内存：包括引擎自身的
# 进程和负责监管它们的驱动。若空闲内存低于该阈值，就没有空间
# 再启动整个运行时；页面缓存会被逐出并引发连锁故障，
# 导致其它无关的操作也跟着变慢。
...
class BrowserTool(BaseTool):
    ...
    def _validate_url_safe(url: str) -> None:
        ...
        for family, _, _, _, sockaddr in addr_infos:
            ...
            # 只拦截高风险目标：链路本地地址（包括
            # 169.254.169.254 云元数据端点）和 IPv6 元数据
            # 地址。本地开发仍可访问环回与 RFC1918/LAN。
    ...
    def _check_engine_ready(self) -> Optional[ToolResult]:
        ...
        # 桌面客户端（开发版或打包版）没有 `cow` CLI —— 需通过聊天中的
        # `/install-browser` 命令来安装；源码 / 网络 / 服务器等其它安装
        # 方式，则改用终端命令 `cow install-browser`。
        install_hint = (
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # 预检：桌面端捆绑了 Playwright 包，但浏览器二进制文件
        # 可能缺失；此时应返回可操作的安装引导，而不是晦涩的
        # 启动失败信息。
        not_ready = self._check_engine_ready()
        if not_ready is not None:
            return not_ready

        # 光有引擎还不够：若在空闲内存不足时强行启动，
        # 会拖慢运行时中的其它一切，因此这里也需要一并检查。
    ...
    def _do_navigate(self, args: Dict[str, Any]) -> ToolResult:
        ...
        # 仅对裸主机自动添加 https://；保留 file://、about:、data: 等。
        if "://" not in url and not url.startswith(("about:", "data:")):
            url = "https://" + url
        # SSRF 防护：对 http(s) 目标，在浏览器导航（随后会把页面快照
        # 回传模型）之前，拒绝解析为链路本地/云元数据地址的 URL。
        # 环回和 RFC1918/LAN 目标被放行，以便本地开发服务器可用。
        # 非 HTTP 方案（about:/data:/file:/chrome:）在此不构成网络出口。
    ...
    def _do_scroll(self, args: Dict[str, Any]) -> ToolResult:
        ...
        amount = args.get("timeout", 500)  # 重用超时字段或默认值
        if "amount" in args:
            amount = args["amount"]
