...
class ChromeLauncher:
    ...
    def __init__(self, executable: str, user_data_dir: str,
        ...
        # 当我们附加到一个并非自己启动的 Chrome 时，设置该字段（而不是 _proc）。
    ...
    def _clear_stale_singleton_locks(self):
        ...
        for name in ("SingletonLock", "SingletonSocket", "SingletonCookie"):
            ...
            try:
                # 这些是符号链接；要判断是否指向悬空目标，得用 lexists。
    ...
    def launch(self, ready_timeout: float = 25.0, adopt: bool = True) -> str:
        ...
        # 我们的 Chrome 是独立启动的，因此它比单次服务重启活得更久。
        # 启动前必须先处理掉任何仍占用该配置目录的进程，
        # 否则两个浏览器最终会共享同一个配置目录。
    ...
    def _spawn_and_wait(self, ready_timeout: float):
        ...
        args = [
            ...
            # 削减首次运行的开销与后台无谓的联网活动，以加快启动速度。
            "--no-first-run",
            "--no-default-browser-check",
            "--disable-background-networking",
            "--disable-component-update",
            "--disable-features=Translate,OptimizationHints",
            # 空白的第一个选项卡使启动成本低廉且可预测。
            "about:blank",
        ]
        if self._headless:
        ...
        popen_kwargs = {}
        if sys.platform == "win32":
            # 与任何控制台分离，并且永远不要在 Windows 上闪烁窗口。
            popen_kwargs["creationflags"] = (
                getattr(subprocess, "CREATE_NO_WINDOW", 0)
                | getattr(subprocess, "DETACHED_PROCESS", 0)
            )
        else:
            # 新会话让子进程脱离父进程的控制终端/进程组，
            # 从而能干净地拆卸，且不会发生信号泄漏。
