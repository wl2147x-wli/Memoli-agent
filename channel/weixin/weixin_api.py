...
# 针对暂时性传输错误的出站 sendMessage 调用的重试策略。
SEND_RETRIES = 2
SEND_RETRY_BACKOFF_BASE = 1.0


# 微信 CDN 仅接受旧版 TLS1.2 + RSA 密码套件（例如
# AES256-GCM-SHA384）。 OpenSSL 1.1.1 的默认 SECLEVEL=2 拒绝这些
# 无前向保密套件，因此握手失败并显示 SSLV3_ALERT_
# 握手失败。仅针对 CDN 请求降低安全级别。
_cdn_session = None
...
# iLink-App-ClientVersion：uint32 编码为 Major<<16 |次要<<8 |补丁
...
def upload_media_to_cdn(api: WeixinApi, file_path: str, to_user_id: str,
    ...
    for attempt in range(1, UPLOAD_MAX_RETRIES + 1):
        try:
            ...
            # API 可能返回 upload_full_url（新）或 upload_param（旧）
...
def download_media_from_cdn(cdn_base_url: str, encrypt_query_param: str,
    ...
    # 确定密钥格式：
    # 1) 32 个字符的十六进制字符串 → 16 个原始字节
    # 2）base64字符串→解码→如果是32字节，则视为十六进制编码→16个原始字节
    # 3）base64字符串→解码→直接16个原始字节
    try:
