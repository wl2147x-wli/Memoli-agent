...
class WeixinChannel(ChatChannel):
    ...
    # ilink bot协议没有出站语音项；以文件形式交付 TTS。
    ...
    def __init__(self):
        ...
        # user_id -> context_token。由 _context_tokens_lock 保护任何
        # 与磁盘持久性竞争的突变。
        self._context_tokens = {}
        self._context_tokens_lock = threading.Lock()
        self._received_msgs = ExpiredDict(60 * 60 * 7.1)
        # session_id -> 截止时间(ts) 直到传入图像/文件
        # 预计完成下载到文件缓存中。来个文字
        # 与图像一起（但稍微提前）到达的消息
        # 等待媒体路径，而不是在没有媒体路径的情况下到达代理。
        self._pending_media = {}
    ...
    def startup(self):
        ...
        # 隔离每个实例的令牌文件，以便两个微信帐户同时运行
        # 一个进程不会覆盖彼此的凭据。实例 ID 是
        # 对于单实例（旧）通道为空 -> 旧路径。
        self._credentials_path = get_weixin_credentials_path(
            getattr(self, "instance_id", "") or ""
        )

        # 始终加载凭据，以便即使在以下情况下我们也可以恢复 context_tokens
        # 机器人令牌本身来自配置。
        ...
        # 通过 QR 流程扫描令牌的实例的回退：
        # flow 写入默认（无 id）凭证文件，因此第一个
        # 要启动的实例在这里继承它而不是被推回
        # 全新扫描。声明一次 - 复制到该实例自己的文件中 - 所以
        # 稍后重新启动时会直接从每个实例的路径中读取它。
        ...
        # 恢复持久化的 context_tokens 以便调度程序可以发送推送
        # 重启后立即，无需等待用户 ping
        # 首先是机器人。
    ...
    def _relogin(self) -> bool:
        ...
        # 有意清除整个凭据文件：新登录
        # 将发出新的 `token` 并持久化 context_tokens 属于
        # 之前的机器人身份，所以他们肯定无法生存。
    ...
    # ── 上下文令牌持久化──────────────────────────────────────
    # ilink 要求每次出站发送都回显 context_token
    # 用户最新的入站消息。我们将内存中的映射镜像到
    # 凭证 JSON 使得计划的推送在进程重新启动后仍然有效。
    # 所有突变+磁盘IO都是通过_context_tokens_lock序列化的，这样
    # 并发更新永远不会丢失彼此的写入。
    ...
    def _print_qr(qrcode_url: str):
        ...
        try:
            ...
            try:
                ...
                # Windows GBK 终端无法渲染 Unicode 块字符
                print(f"\n  (终端不支持显示二维码，请使用链接扫码)")
    ...
    def _poll_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # 处理消息。媒体（图像/文件/视频）消息是
                # 在同一批次中的文本之前处理，以便
                # 与图像一起发送的短信会找到图像
                # 已经在缓存中（否则媒体下载会让
                # 文本赢得比赛并在没有媒体的情况下到达代理人）。
                msgs = resp.get("msgs", [])
    ...
    def _process_message(self, raw_msg: dict):
        ...
        if msg_type != 1:  # 只处理USER消息（type=1）
            return
        ...
        # 在（慢速）同步下载*之前*将媒体标记为正在传输，以便
        # 首先处理的来自同一用户的短信可以等待
        # 它（请参阅_wait_for_pending_media）。
        ...
        if wx_msg.ctype == ContextType.TEXT:
            if has_media:
                # 该消息已经内嵌了自己的媒体；它的参考是
                # 在解析期间附加，因此只需释放飞行中的标记即可。
                self._clear_pending_media(session_id)
            else:
                # 来自同一用户的媒体消息可能仍在下载（
                # 可以以单独的轮询批次到达）；稍等片刻，以便其路径
                # 被附加，而不是代理在没有图像的情况下回答。
    ...
    def send(self, reply: Reply, context: Context):
        ...
        # 媒体回复可以携带附带消息（代理的摘要）
        # 刚刚发送的文件）。 ilink 没有标题字段，因此它会作为
        # 先分离气泡。图片回复被跳过：chat_channel 已经
        # 在我们到达这里之前将他们的文本分开，再做一次就会
        # 复制气泡。
