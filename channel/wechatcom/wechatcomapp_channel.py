...
class WechatComAppChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        # 当通道已启用但其状态不佳时，快速失败并显示可读消息
        # 缺少所需的凭据。否则微信加密连接
        # 带有字符串的 None aes_key 并引发不透明
        # “+ 不支持的操作数类型：‘NoneType’和‘str’”。
        missing = [
            key
            for key, val in (
    ...
    def POST(self):
        ...
        if msg.type == "event":
            if msg.event == "subscribe":
                ...
                # 回复内容 = 订阅消息()
                # 如果回复内容：
                #     回复 = create_reply(reply_content, msg).render()
                #     res = channel.crypto.encrypt_message(回复、随机数、时间戳)
                #     返回资源
        else:
            try:
