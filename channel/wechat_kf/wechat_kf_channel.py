...
except ImportError as e:  # 语音功能可选
    logger.debug(
...
class WechatKfChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        # 使用库存wechatpyWeChatClient，这样access_token是
        # 缓存并仅在实际过期时（~2 小时）刷新。当地的
        # `WechatComAppClient` 子类的背景刷新已损坏
        # 每 60 秒重新获取一次的循环和 `fetch_access_token()`
        # 覆盖可能返回一个字典而不是字符串，其中
        # 损坏 URL 并触发错误代码 40014。
        self.client = WeChatClient(self.corp_id, self.secret)

        # 默认情况下将sync_msg光标保留在用户的主目录下，
        # 因此它可以在 `tmp/` 清理和重启后的 cwd 更改中幸存下来。
        ...
        # WeCom 要求回调 HTTP 响应在 ~5 秒内返回，
        # 否则它会重试相同的通知。 sync_msg 拉取
        # 很容易超过这个值，因此我们将其分派到后台池
        # 并让`Query.POST`立即回复成功。
        self._callback_executor = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="wxkf-cb"
        )
        # Per-open_kfid 锁：为同一个 kf 账户序列化sync_msg
        # 这样回调重试（或快速事件）就不会继续进行
        # 相同的光标并产生重复的回复。
    ...
    def _run_callback(self, token: str, open_kfid: str):
        # 阻止每个 kfid 锁，以便重试回调排队
        # 在飞行中的后面。然后排队的工作人员将调用
        # 带有（已经高级的）游标的sync_msg，这很便宜
        # 当没有新消息但仍收到任何消息时
        # 在前一个工人最后一次拉动之后到达。
    ...
    def consume_callback(self, token: str, open_kfid: str):
        ...
        for raw in msgs:
            ...
            # 在文本转弯时，附加任何待处理的图像/文件作为参考
            # 这样下游代理就可以通过文本内容来获取它们。
            # 路径已位于 agent_workspace/tmp 下（请参阅
            # WechatKfMessage._get_tmp_dir)，因此相对引用也可以。
    ...
    def _call_sync_msg(self, token: str, open_kfid: str, cursor: str) -> Optional[dict]:
        # `client.access_token` 是缓存的字符串属性；不要使用
        # `fetch_access_token()` 这里 - wechatpy 返回原始响应
        # 来自该调用的 dict，这会损坏查询字符串。
...
class Query:
    ...
    def POST(self):
        ...
        # 我们需要内部 XML 中的 Token + OpenKfId 字段来调用
        # 同步消息。 wechatpy 的解析对象都没有暴露，所以我们解析
        # 直接原始 XML。
        try:
        ...
        # 移交给后台工作人员 — WeCom 需要回调
        # 在〜5秒内返回成功，否则将重试
        # 我们可能会在同一个光标窗口中出现重复的回复。
