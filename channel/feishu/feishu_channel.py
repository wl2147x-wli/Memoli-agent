...
# 延迟导入lark_oapi SDK。完整的 `import lark_oapi` 吸引了 10k+
# 文件并需要 4-10 秒，因此我们将实际导入推迟到需要的地方。
# 在桌面模式下，SDK 不捆绑；第一次导入下载一个修剪过的
# 捆绑到每个用户可写的目录中（参见channel/feishu/lark_install.py）。
from channel.feishu import lark_install

lark = None  # 将在首次使用时通过 _ensure_lark_imported() 进行填充
...
def _new_lark_ws_client_module(loop):
    ...
    # 在规范名称下执行，以便加载程序接受它和相关名称
    # 导入解析，然后重命名副本，使其与缓存的不同
    # 模块（我们故意不在 sys.modules 中注册它）。
...
class FeiShuChanel(ChatChannel):
    # 类级默认值，在导入时读取一次旧的单实例
    # 案例。通过在startup()中重新读取每个实例的权威值
    # self.cfg()，在设置时更喜欢此实例的凭据覆盖。
    ...
    # 离线积压的后备：这么旧的重播将被丢弃，即使它
    # 启动后以某种方式到达 create_time 。
    ...
    def __init__(self):
        ...
        self._bot_open_id = None  # 用于@提及匹配的缓存机器人 open_id
        # 当该频道开始服务时。在startup()中设置； 0 表示“未知”，
        # 它让每条消息都通过，而不是默默地丢弃它。
    ...
    def stop(self):
        ...
        # lark.ws.Client 没有 stop() 方法；上面的线程中断就足够了
    ...
    def _startup_websocket(self):
        ...
        def start_client_with_retry():
            ...
            # lark_oapi.ws.client 将其事件循环保留在模块级全局中，
            # 和 Client.start() / _select() / _ping_loop() 都引用它。一个
            # 因此，单个共享模块无法同时托管两个 ws 客户端：
            # 第二个实例覆盖全局循环并且两个客户端都结束
            # 在一个循环上进行比赛（“事件循环已经在运行”/“未来
            # 连接到不同的循环”）。
            #
            # 为每个 ws 线程提供其自己的 ws.client 模块的*私有副本*
            # 有自己的循环，所以有几个飞书实例（每个边界一个
            # Agent）可以各自独立运行一个websocket连接。单个
            # 实例仍然像以前一样工作。
    ...
    def _handle_message_event(self, event: dict):
        ...
        # 删除离线积压的飞书重连后重播，无需
        # 丢弃缓慢但最新的消息。测试是消息所在的位置
        # 相对于这家初创公司，而不是它的年龄：也有绝对年龄限制
        # 杀死了仅仅传递滞后的全新消息（飞书排空
        # 重新连接后的队列），并且没有为时钟偏差留下余量。
        # MAX_AGE 只是重放的一个后备，重放太大以至于比启动的寿命更长。
        ...
        if context:
            # 团队机器人：领先的“@teammate”将这一回合交给该成员，
            # 与 Web 控制台完全相同。从实例名册中解决
            # 直接，所以它适用于第一条消息。
            ...
            # 飞书召回事件仅包含message_id/chat_id。保留
            # 接受路由并使用 message_id 作为代理取消密钥。
    ...
    def _make_feishu_stream_callback_plain(self, context, access_token):
        ...
        # 一旦初始化失败就长期标记为 disabled，本次回复不再尝试任何流式调用
        disabled = [False]
        # 在agent_cancelled后为真：agent_end停止重写卡
        # 过时的 Final_response 并只是最终确定当前内容。
        cancelled = [False]
        lock = threading.Lock()
        ...
        def on_event(event: dict):
            ...
            if event_type == "message_update":
                ...
                # 将通道锁定为“无重写”模式：后续
                # agent_end 的 Final_response 来自最后一个 *completed*
                # 转（用户已经看到了），因此重写卡片
                # 会在视觉上复制它。
                with lock:
                    cancelled[0] = True
    ...
    def _upload_image_url(self, img_url, access_token):
        ...
        if img_url.startswith("file://"):
            local_path = img_url[7:]  # 删除“file://”前缀
    ...
    def _upload_video_url(self, video_url, access_token):
        ...
        try:
            # 对于file:// URL（本地文件），直接上传
            if video_url.startswith("file://"):
                local_path = video_url[7:]  # 删除文件://前缀
                if not os.path.exists(local_path):
            ...
            # 仅在可用时添加持续时间（视频/音频需要）
            if duration:
                data['duration'] = duration  # 必须是 int，而不是 string
            ...
            with open(local_path, "rb") as file:
                ...
                response_data = upload_response.json()
                if response_data.get("code") == 0:
                    # 将持续时间添加到响应数据（API 不返回它）
                    upload_data = response_data.get("data")
                    upload_data['duration'] = duration  # 添加我们计算的持续时间
                    logger.info(
    ...
    def _upload_audio(self, audio_path, access_token):
        ...
        # 飞书仅播放 opus 格式的音频消息。
        # 如果 TTS 引擎生成不同的格式（例如来自 OpenAI TTS 的 mp3），则进行转换。
    ...
    def _upload_file_url(self, file_url, access_token):
        ...
        if file_url.startswith("file://"):
            local_path = file_url[7:]  # 删除“file://”前缀
            logger.info(f"[FeiShu] uploading local file: {local_path}")
            ...
            # 判断飞书API的文件类型
            # 飞书支持：opus、mp4、pdf、doc、xls、ppt、stream（其他类型）
            file_type_map = {
                '.opus': 'opus',
                '.mp4': 'mp4',
                '.pdf': 'pdf',
            ...
            file_type = file_type_map.get(file_ext, 'stream')  # 默认为其他类型进行流式传输
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        # 多实例路由+团队：与基础通道相同的标记，因此
        # 飞书APP绑定了特定的Agent，路由到该Agent并携带其团队。
        ...
        # 根据聊天类型设置session_id
        if cmsg.is_group:
            # 群聊：检查group_shared_session是否启用
            if conf().get("group_shared_session", True):
                # 组中的所有用户共享相同的会话上下文
                context["session_id"] = cmsg.other_user_id  # 组ID
            else:
                # 每个用户在组内都有自己的会话
                # 这可以确保：
                # - 不同组中的同一用户有不同的对话历史
        ...
        if ctype == ContextType.TEXT:
            ...
            # 仅当设置了始终开启开关时，文本输入才会选择语音回复。
            if "desire_rtype" not in context and conf().get("always_reply_voice"):
                context["desire_rtype"] = ReplyType.VOICE

        elif context.type == ContextType.VOICE:
            # 2.语音请求：当 voice_reply_voice（镜像回复）或 always_reply_voice
            # 已开启时，用语音回复语音输入。
...
class FeishuController:
    ...
    def POST(self):
        try:
            ...
            # 2. 验证回调。卡回调可以携带验证
            # token 在 event.token 中，而消息事件则在 header 中携带它。
