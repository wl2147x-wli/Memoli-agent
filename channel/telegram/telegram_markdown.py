...
# 机器人 API 限制。 Telegram 在实体解析后对这些进行计数，因此测量
# 为了安全起见，针对他们的降价源会犯错误（标签不会增加长度）。
TEXT_LIMIT = 4096
CAPTION_LIMIT = 1024
...
_QUOTE_RE = re.compile(r"^\s*&gt;\s?(.*)$")  # 逃脱后匹配

_BOLD_RE = re.compile(r"\*\*(.+?)\*\*")
_BOLD_ALT_RE = re.compile(r"(?<!\w)__(.+?)__(?!\w)")
_STRIKE_RE = re.compile(r"~~(.+?)~~")
# 内部的 \S 保护防止像“2 * 3 * 4”这样的算术被读取为强调。
...
def to_telegram_html(text: str) -> str:
    ...
    # 代码和表格首先呈现并停在占位符后面，因此
    # 下面的内联传递不能将其内容重新解释为格式。
    parked: list[str] = []
