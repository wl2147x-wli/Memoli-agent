...
# 意图：GROUP_AND_C2C_EVENT(1<<25) | PUBLIC_GUILD_MESSAGES(1<<30)
...
class QQChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        # 每次停止都会发生碰撞，因此被取代的套接字无法自行重新连接。
        self._generation = 0
        # 最后一次来自 QQ API 的拒绝，出现在频道页面上，因此
        # 用户看到的是平台的原因，而不是单纯的“不可用”。
    ...
    def startup(self):
        if self._ws is not None:
            # 重新启动通道会在同一实例上再次调用startup()。
            # 让旧套接字保持打开状态将使 QQ 推送每个事件两次。
    ...
    def _refresh_access_token(self):
        try:
            ...
            if not token:
                # 凭证错误会以带业务错误码的 HTTP 200 返回，
                # 例如 ("机器人不存在", "appid invalid")，因此 status 中不会有任何提示。
                # 返回而不触及到期时间：下一次调用重试，并且
                # 仍然有效的令牌不会因为小问题而被丢弃。
    ...
    def _get_ws_url(self) -> str:
        try:
            ...
            if resp.status_code != 200:
                # body携带平台自己的code/message/trace_id，
                # 唯一区分 IP 允许列表拒绝的因素
                # 来自不良令牌或未发布的机器人。
