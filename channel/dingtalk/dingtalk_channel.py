...
class DingTalkChanel(ChatChannel, dingtalk_stream.ChatbotHandler):
    ...
    def startup(self):
        ...
        # 我们自己运行连接循环，而不是委托给 client.start()，
        # 这样我们就可以得到详细的错误消息并快速响应 stop()。
        ...
        while self._running:
            ...
            try:
                # 钉钉的流式网关是直连的：绝不通过 OS/环境变量中的
                # SOCKS 代理路由。websockets >= 13 会自动检测到
                # ALL_PROXY/all_proxy，并因缺少 “python-socks” 这个额外依赖
                # 而直接失败，所以这里显式地选择不使用代理。
                # 旧版 websockets 没有 `proxy` 参数，且从不走代理，因此可优雅回退。
                connect_kwargs = {}
                try:
    ...
    def handle_group(self, cmsg: DingTalkMessage):
        ...
        if conf().get("group_shared_session", True):
            session_id = cmsg.other_user_id  # 对话id
        else:
    ...
    def send(self, reply: Reply, context: Context):
        ...
        # 检查 msg 是否存在（对于计划任务，msg 可能为 None）
        ...
        # 本机样本音频。上传仅接受 ogg/amr，因此将 TTS mp3/wav 转换为 amr。
