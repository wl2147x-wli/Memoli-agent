...
MEDIA_CHUNK_SIZE = 512 * 1024  # 每个块 512KB（base64 编码之前）
# 修复了回调 (webhook) HTTP 服务器的 URL 路径。机器人的
# 接收消息 URL 必须指向此路径，例如http://host:9892/wecombot
...
class WecomBotChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        # 传输模式：“websocket”（长连接）或“webhook”（HTTP回调）
        self.mode = "websocket"
        self._crypt = None
        self._http_server = None
        # Stream_id -> {“已提交”，“当前”，“已完成”，“图像”，“最后访问”}
        self._callback_streams = ExpiredDict(60 * 10)  # 10 分钟后自动过期（最大轮询窗口为 6 分钟）
    ...
    def startup(self):
        ...
        # Webhook 模式绑定固定的 HTTP 端口，因此不能托管多个
        # 进程中的实例。只有websocket传输是多实例的
        # 准备好了；拒绝启动绑定（多实例）webhook 机器人而不是
        # 与另一个实例在端口上无声地发生冲突。
        if self.mode == "webhook":
            # Webhook 绑定固定的 HTTP 端口，因此只能运行一个 webhook bot
            # 每个进程。默认（引导）实例保留
            # instance_id == channel_type 并且是允许的 — 它仍然是那个
            # webhook bot，仅携带代理绑定。一个*额外*实例
            # （一个不同的instance_id）会在端口上发生冲突，所以拒绝它
            # 并将用户指向 websocket 模式。
    ...
    def _new_callback_stream(self, response_url: str = "") -> str:
        ...
        with self._callback_lock:
            self._callback_streams[stream_id] = {
                ...
                "current": "",
                "finished": False,
                "images": [],  # (base64_str, md5_str) 列表，仅在完成时刷新
                "image_urls": [],  # 公共http(s)图像url（可在response_url markdown中使用）
                "image_pending": False,  # 正在准备图像回复；尚未完成文字
                "last_access": now,
                "created_at": now,
                "response_url": response_url or "",
                "delivered": False,  # 最终答案通过民意调查交给WeCom
                "url_sent": False,   # 通过response_url推送的最终答案（主动回复）
            }
        return stream_id
    ...
    def _callback_handle_message(self, data: dict) -> dict:
        ...
        # response_url 让我们在 1 小时内主动回复一次，用作后备
        # 当代理在 WeCom 停止轮询后完成时（最多约 6 分钟窗口）。
    ...
    def _callback_handle_stream_poll(self, data: dict) -> dict:
        ...
        with self._callback_lock:
            ...
            if state is None:
                # 未知/过期流：告诉 WeCom 我们已完成停止轮询。
                return {"msgtype": "stream", "stream": {"id": stream_id, "finish": True, "content": ""}}
            state["last_access"] = time.time()
            if state.get("url_sent"):
                # 最终答案已通过response_url推送；默默地结束。
                return {"msgtype": "stream", "stream": {"id": stream_id, "finish": True, "content": ""}}
            # 我们从不强制完成计时器：当任务仍在运行时
            # 气泡应该继续旋转，直到任务完成或
            # 用户取消。如果 WeCom 的 6 分钟窗口在完成之前关闭，
            # 稍后将通过response_url 传递答案。
    ...
    # ------------------------------------------------------------------
    # 回调（webhook）发送：把最终回复写入流状态，
    # 以便下一次 “流式消息刷新” 轮询能以 finish=true 将其返回。
    # ------------------------------------------------------------------
    ...
    def _callback_finalize_text(self, stream_id: str, content: str, append: bool = False):
        with self._callback_lock:
            ...
            state["current"] = ""
            state["last_access"] = time.time()
        # 不要同步完成：chat_channel 分割带有标题的图像
        # 回复 TEXT 发送，然后（0.3 秒后）发送 IMAGE。如果
        # text 立即结束流，WeCom 会在
        # 图像到达。推迟完成，以便尾随图像可以合并进来。
    ...
    def _callback_finalize_image(self, stream_id: str, img_path_or_url: str):
        ...
        with self._callback_lock:
            ...
            if b64md5:
                ...
                state["committed"] = accumulated
                # 记住公共 url（如果有），以便使用 response_url 进行回退
                # 当投票窗口关闭时，可以将其嵌入为降价。
    ...
    # ------------------------------------------------------------------
    # 主动回复回退 (response_url)：之后完成的救援回复
    # WeCom 停止轮询（被动流窗口距用户的时间大约 6 分钟）
    # 消息）。短暂的延迟可以让飞行中的民意调查首先进行；仅当没有
    # poll 获取完成的答案，我们是否通过 response_url 主动推送它。
    # ------------------------------------------------------------------
    ...
    def _load_image_base64(self, img_path_or_url: str):
        ...
        # 我们在这里创建的临时文件（下载/转换/压缩）必须是
        # 之后清理干净；调用者的原始本地文件一定不是。
        temp_files = []
        try:
            ...
            # 与长连接路径（只上传和发送很小的数据）不同
            # media_id)，回调回复将整个图像作为base64嵌入其中
            # 每次轮询都会返回的 AES 加密正文。经验上
            # ~1.5MB 图像（base64 ~2.1MB，加密 ~2.8MB）使 WeCom 拒绝
            # 永远完成数据包和轮询，因此上限远低于该值。
    ...
    def send(self, reply: Reply, context: Context):
        ...
        # 确定 req_id 进行响应或使用 send_msg 进行计划推送
    ...
    def _send_image(self, img_path_or_url: str, receiver: str, is_group: bool, req_id: str = None):
        ...
        max_image_size = 2 * 1024 * 1024  # 图片上传限制为 2MB
...
class WecomBotCallbackController:
    ...
    def POST(self):
        ...
        # 流民意调查到达约 1/s；记录每个记录都很吵，所以只记录非轮询
        # 此处回调（轮询完成记录在流轮询处理程序中）。
        if msgtype != "stream":
