...
function createDocEditor(cfg) {
    // 屏幕上的文档：无论主页使用什么形状，加上
    // 此编辑器在加载和保存时保持当前的 `content` 字段。
    let doc = null;
    let editing = false;
    let baseline = '';
    let baseMtime = null;
    ...
    async function start() {
        ...
        // 从文本区域读回基线，而不是使用
        // 响应文本：文本区域将其值中的 CRLF 标准化为 LF，因此
        // CRLF 文件将从加载那一刻起进行比较。
    ...
    async function save(opts) {
        ...
        // Ctrl+S 绕过按钮的忙碌状态，并发送第二个保存
        // 在第一个回复带有陈旧的时间之前 - 它会回来
        // 作为与我们自己写的冲突。
        if (saving) return;
