# 接入LinkAI代理平台
# 文档：https://docs.cowagent.ai/zh/models/linkai
...
class LinkAIBot(Bot, OpenAICompatibleBot):
    ...
    def get_api_config(self):
        ...
        return {
            'api_key': conf().get("open_ai_api_key"),  # LinkAI 使用 OpenAI 兼容密钥
...
# 将 call_with_tools 方法添加到 LinkAIBot 类
def _linkai_call_with_tools(self, messages, tools=None, stream=False, **kwargs):
    ...
    try:
        ...
        if reasoning_effort:
            ...
            # LinkAI将请求转发给不同的上游供应商。门
            # 通过经过验证的模型功能传递字段，因此类似于 OpenAI
            # 模型不会接收未知参数。
        ...
        # Kimi K3使用顶级reasoning_effort并拒绝泛型
        # 较旧的 Kimi/DeepSeek 兼容路径使用的思考对象。
        ...
        for line in res.iter_lines():
            if line:
                ...
                if line.startswith('data: '):
                    ...
                    # 检查流中的错误响应
                    # 一些提供商（例如，通过 LinkAI 的 MiniMax）返回错误如下：
                    # {'类型'：'错误'，'错误'：{'类型'：'...'，'消息'：'...'，'http_code'：'400'}}
                    ...
                    # 按原样转发 SSE JSON 扩展（例如 delta._gemini_raw_parts
                    # 对于 Gemini，通过 LinkAI）到达 agent_stream 并存储在助手上
                    # 下一个请求的消息。标准 OpenAI 字段保持不变。
                    yield chunk
...
def _linkai_convert_messages_to_openai_format(self, messages):
    ...
    # DeepSeek（通过 LinkAI 代理）需要 `reasoning_content`
    # 一旦历史记录包含任何工具调用转向，则助理消息 - 不
    # 只是工具调用本身。首先检测该情况。
