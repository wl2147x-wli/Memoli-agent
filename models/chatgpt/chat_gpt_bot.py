...
class ChatGPTBot(Bot, OpenAIImage, OpenAICompatibleBot):
    def __init__(self):
        super().__init__()
        # 从配置中解析 api key/base（不再有全局 SDK 状态）。
        is_custom, _ = parse_custom_bot_type(conf().get("bot_type", ""))
        custom_model = None
        if is_custom:
            # 通过 bot_type“custom:<id>”支持多个自定义提供程序
            # 自动回退到旧的 custom_api_key/base 字段。
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            #     # 在流中回复
            #     返回 self.reply_text_stream(查询、new_query、session_id)
    ...
    def reply_text(self, session: ChatGPTSession, api_key=None, args=None, retry_count=0) -> dict:
        ...
        try:
            ...
            # 如果 api_key 为 None，则将使用每个实例的默认密钥。
            if args is None:
                args = self.args
            # 将旧的 SDK kwargs 转换为 HTTP 客户端参数：
            # - request_timeout / 超时 -> 每次调用超时
            call_args = dict(args)
...
class AzureChatGPTBot(ChatGPTBot):
    ...
    def get_api_config(self):
        ...
        # Azure HTTP 客户端已存储部署范围的基本 URL。
        # 再次传递原始端点将在 call_with_tools() 中覆盖它。
...
class _AzureChatHTTPClient(OpenAIHTTPClient):
    ...
    def _build_headers(self, api_key, extra_headers, url=None):
        # Azure 使用 api-key 标头，而不是 Bearer 令牌。没有归属
        # headers — Azure 部署是客户自己的租户。
