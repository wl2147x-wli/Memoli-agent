...
# 代理模式下V4模型的输出预算。省略 max_tokens 会留下 API
# 默认值远低于 V4 可以发出的值 (384K)，这会切断工具调用
# 关于大文件写入的中间争论 - 然后整个生成被丢弃。
# 这是上限，而不是预留：未使用的预算不产生任何费用。
...
class DeepSeekBot(Bot, OpenAICompatibleBot):
    ...
    def call_with_tools(self, messages, tools=None, stream: bool = False, **kwargs):
        ...
        try:
            ...
            if self._model_supports_thinking(model):
                ...
                if thinking_active:
                    # 默认为“高”；允许调用者覆盖（例如，重代理循环的“max”）。
                    request_body["reasoning_effort"] = reasoning_effort or "high"
            elif self._is_reasoner_model(model):
                # R1 无条件思考 — 没有 `thinking` 字段，但 Reasoning_content 仍然流动。
                thinking_active = True

            # 在思考模式下默默地忽略剥离参数以保持电线清洁。
            if thinking_active:
                for k in ("temperature", "top_p", "presence_penalty", "frequency_penalty"):
    ...
    def _handle_stream_response(self, request_body: dict):
        ...
        try:
            ...
            for line in response.iter_lines():
                ...
                # 推理内容（思维方式）。作为自己的增量转发，所以
                # agent_stream.py 可以将其拼接成 `thinking` 块。
    ...
    def _handle_sync_response(self, request_body: dict):
        ...
        try:
            ...
            # 将表面推理作为 `thinking` 块，以便代理层可以将其持久化
            # 并在工具调用轮流中往返（DeepSeek API 需要）。
    ...
    def _convert_messages_to_openai_format(self, messages):
        ...
        # 判断历史记录中是否存在工具调用助手轮次。
        # 如果是这样，则每个辅助消息必须携带`reasoning_content`。
        ...
        for msg in messages:
            ...
            if role == "user":
                ...
                # 往返推理_内容：每个助手都需要
                # 一旦历史记录包含任何工具调用回合，就会显示消息（请参阅
                # 外部评论）。当跟踪时使用空字符串作为后备
                # 未被捕获 — DeepSeek 验证现场存在，而不是
                # 价值；不思考的后端会默默地忽略它。
                if reasoning_parts:
