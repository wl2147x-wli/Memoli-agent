...
# 2020，林志军，https://github.com/toolgood/ToolGood.Words
...
class WordsSearch():
    ...
    def SetKeywords(self,keywords):
        ...
        # 第一个 = []
        # 对于范围内的索引(65535):# for (index = 0;index < 0xffff;index++) 
        #     首先.追加（无）
        
        # 对于 allNode2[0].m_values 中的键：
        #     第一个[key] = allNode2[0].m_values[key]
    ...
    def FindFirst(self,text):
        ptr = None
        for index in range(len(text)): # for (索引 = 0; 索引 < 文本.长度; 索引++) 
            t =ord(text[index]) # text.charCodeAt(索引)
            tn = None
            if (ptr == None):
    ...
    def Replace(self,text, replaceChar = '*'):
        ...
        ptr = None
        for i in range(len(text)): # for (i = 0; i < 文本长度; i++) 
            t =ord(text[i]) # text.charCodeAt(索引)
            tn = None
            if (ptr == None):
                tn = self._first.TryGetValue(t)
            else:
            ...
            if (tn != None):
                if (tn.End):
                    ...
                    start = i + 1 - maxLength
                    for j in range(start,i+1): # for (j = 开始; j <= i; j++) 
