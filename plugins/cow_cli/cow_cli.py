...
# 已知cow 支持的顶级子命令。
# “start”/“stop”/“restart”指的是主机 shell 上的守护进程生命周期；
# 在聊天中，“/cancel”会中止正在运行的代理运行。
KNOWN_COMMANDS = {
...
# 只能从 CLI（终端）运行的命令，不能在聊天中运行。
CLI_ONLY_COMMANDS = {"start", "stop", "restart"}

# 只能从聊天中运行的命令（需要访问进程内内存）
CHAT_ONLY_COMMANDS = set()  # 两者都允许上下文，但行为不同

# 仅斜杠形式的方便简写。值是*完整命令
# strings* 因此别名可以携带参数（例如“cc”->“上下文清除”）。
# 这些简写是经过深思熟虑的，不能从前缀/拼写错误规则中得出：
#   - “c” 是 cancel/config/context 的前缀（不明确） -> 需要显式映射
#   - “cc” 是无前缀，并扩展为命令 + 参数
#   - 否则“s”将是一个不明确的前缀（技能/开始/状态/停止）
# 用户可以通过 config.json“command_aliases”覆盖/扩展它们。
DEFAULT_ALIASES = {
    "c":   "cancel",
    "cc":  "clear",
    "cp":  "compact",
...
class CowCliPlugin(Plugin):
    ...
    def _parse_command(self, content: str):
        ...
        if content.startswith("cow "):
            ...
            if cmd not in KNOWN_COMMANDS:
                # 'cow xxx' 不是一个命令 —— 不要拦截（可能是
                # "cow xxx 怎么样" 之类的自然语言输入）。
                return None
            args = parts[1] if len(parts) > 1 else ""
            return cmd, args
    ...
    def _resolve(self, token: str, user_args: str):
        ...
        # 1. 精确命令（真正的命令永远不会被别名所掩盖）
        if token in KNOWN_COMMANDS:
            return ("run", token, user_args)

        # 2. 别名 -> 完整命令字符串；将别名参数与用户参数合并。
        #    别名扩展最多应用一次（无别名 -> 别名链）。
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        ...
        return self._typo_hint(token, result[1])  # “错别字”
    ...
    # ------------------------------------------------------------------
    # 任务 - 只读调度程序列表，范围仅限于当前聊天。
    # 飞书在该插件之前拦截/tasks以保留其交互卡；
    # 所有其他频道都会收到此纯文本视图。
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 取消 — 中止当前会话的运行中代理程序运行。
    # 后备处理程序；在实践中，chat_channel/web_channel 拦截
    # /cancel 较早，因此它绕过每个会话串行队列。
    # ------------------------------------------------------------------
    ...
    def _cmd_cancel(self, args: str, e_context: EventContext, session_id: str = "", **_) -> str:
        ...
        # 首选每回合 request_id （与注册的 key agent_bridge 匹配）
        cancelled = 0
        request_id = ""
        if e_context is not None:
            try:
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        # 写入数据目录（桌面上的~/.cow，否则为项目根目录），而不是
        # __file__-relative — 后者位于只读 .app 包中
        # 当冻结并在那里写入时，会破坏 macOS 代码签名密封。
        ...
        # 将更新的值同步到环境变量，以便 load_config()
        # 不会用过时的环境变量覆盖新值（在 Docker 中常见）。
        # 不区分大小写地匹配环境变量键（Docker compose 通常使用
        # 像 MODEL 一样大写，但也可以使用小写）。
        synced_envs = {}
    ...
    # ------------------------------------------------------------------
    # install-browser（与cow install-browser CLI 共享逻辑）
    # ------------------------------------------------------------------
    ...
    def _skill_list_local(self, agent_id=None) -> str:
        ...
        # 此代理的选择范围：在 team.json 中选择的子集意味着
        # /这里的技能列表仅显示该特工实际使用的内容，而不是
        # 整个共享库。 ``None``选择=每个共享技能；一个空的
        # 列表是故意的“无”，并且不能落入磁盘扫描。
    ...
    def _skill_uninstall(self, name: str) -> str:
        ...
        # 该技能已从共享库中消失，因此请从任何内容中删除其名称
        # team.json 中的代理选择列表，而不是留下死条目。
        try:
    ...
    def _memory_dream(self, days: int, e_context, session_id: str) -> str:
        ...
        # SaaS（e_context为None）：同步运行，返回完整结果
        if e_context is None:
            return self._memory_dream_sync(flush_mgr, days)

        # 本地通道：在后台运行，通过channel.send()通知
    ...
    def _resolve_active_embedding():
        ...
        if not provider_key:
            # 旧版自动路径：openai -> linkai，均默认为 text-embedding-3-small (1536)。
        ...
        # 由于我们添加了对矢量模型的自定义提供程序的支持，因此这部分应进行相应修改：
        # 自定义提供程序（“custom:<id>”）解析为“自定义”供应商密钥。
        ...
        # 自定义提供程序模型后备：从 custom_providers 条目读取。
    ...
    def _memory_status(self) -> str:
        ...
        # 从最新的配置中解析，以便用户看到/内存重建索引
        # 实际上将运行为 — 而不是缓存代理初始化时使用的运行方式。
        ...
        # 健康提示——仅在用户明确选择加入时显示
        # 通过 `embedding_provider` 进行矢量搜索。旧用户（没有明确的
        # 提供商）按设计以“尽力而为向量”模式运行；
        # 唠叨他们缺失/不匹配的向量将是噪音。
        warnings = []
    ...
    def _memory_rebuild_index(self, e_context, session_id: str) -> str:
        ...
        # 根据最新配置重建：构建一个新的提供程序
        # config.json 并将其交换到内存管理器上。代理的
        # conversation_history 和其他状态保持不变。
        try:
        ...
        # SaaS（e_context为None）：同步运行，返回最终结果
        if e_context is None:
    ...
    def _knowledge_toggle(self, enabled: bool) -> str:
        ...
        # 数据目录（桌面中的 ~/.cow），而不是 __file__ 相关的 — 请参阅 _config_set。
        from config import get_data_root
        config_path = os.path.join(get_data_root(), "config.json")
        try:
            with open(config_path, "r", encoding="utf-8") as f:
