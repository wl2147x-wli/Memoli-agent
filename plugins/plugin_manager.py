...
class PluginManager:
    ...
    def load_config(self):
        ...
        modified = False
        # 首选可写副本（数据目录）；退回到发货时的状态
        # 资源目录（在保存之前首先在冻结的包中运行）。
    ...
    # 仅 IM 频道插件在单用户桌面应用程序中没有任何意义
    # 更糟糕的是，在 init 时将 config.json 写入其包目录中 — 这会破坏
    # 打包的 .app 的 macOS 代码签名密封。 cop_cli 保持启用状态
    # 所以桌面聊天命令（/status，/help，...）继续工作。
