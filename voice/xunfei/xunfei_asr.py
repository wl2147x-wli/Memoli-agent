...
class Ws_Param(object):
    ...
    def __init__(self, APPID, APIKey, APISecret,BusinessArgs, AudioFile):
        ...
        #self.BusinessArgs = {"域名": "iat", "语言": "zh_cn", "口音": "普通话", "vinfo":1,"vad_eos":10000}
        ...
        if code != 0:
            ...
            if "rg" in temp1.keys():
                ...
                for sn in range(rep_start,rep_end+1):
                    #print("弹出之前",whole_dict)
                    #打印（“SN”，SN）
                    whole_dict.pop(sn,None)
                    #print("弹出后",whole_dict)
                results = ""
                for i in data:
                    for w in i["cw"]:
                        results += w["w"]
                whole_dict[sn]=results
                #print("添加后",whole_dict)
            else:
                results = ""
                for i in data:
                    for w in i["cw"]:
                        results += w["w"]
                whole_dict[sn]=results
            #print("sid:%s 调用成功！,数据为:%s" % (sid, json.dumps(data, Ensure_ascii=False)))
